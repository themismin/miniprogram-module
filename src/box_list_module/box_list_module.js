 ...
 */
Component({
  behaviors: [boxbehavior, skinBehavior],
  properties: {
    /**
     * 列表盒子数据
     * @property {Object} boxdata - 包含列表项信息的对象
     */
    boxdata: {
      type: Object,
      /**
       * 观察boxdata属性变化并处理列表项图片
        ...
        if (Object.keys(newVal).length > 0) {
          if (newVal.module_contents != null && newVal.module_contents.length > 0) {
            newVal.module_contents.forEach((w, j) => {
              newVal.module_contents[j].content = app.cropimg(w.content, 30, 30, 'crop', 1.5)
            })
          }
          that.setData({
            box: newVal,
