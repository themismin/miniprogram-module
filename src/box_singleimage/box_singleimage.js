...
Component({
  properties: {
    /**
     * 盒子组件数据
     * @property {Object} boxdata - 包含图片和设置信息的对象
     */
    boxdata: {
      type: Object,
      /**
      ...
      observer: function (newVal, oldVal, changedPath) {
        ...
        if (Object.keys(newVal).length > 0) {
          if (newVal.module_contents != null && newVal.module_contents != '') {
            newVal.module_contents.content = app.cropimg(newVal.module_contents.content, newVal.module_contents.attrs.width, newVal.module_contents.attrs.height, 'crop', 1.5)
            that.setData({
              box: newVal,
            })
          }
